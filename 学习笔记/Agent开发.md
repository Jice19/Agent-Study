...
>**不要把检索评估和端到端 RAG 效果混为一谈**：必须先单独评估检索环节，否则无法区分「回答错误是因为没检索到正确内容，还是 LLM 没用到正确内容」。
>
>**不要只看单一指标**：不能只追求高召回率，忽略精准率 ——Top20 里有 1 个正确内容，召回率 100%，但给 LLM 的上下文 95% 都是噪音，反而会导致幻觉。
>
>**不要用合成数据完全替代真实数据**：合成数据和真实用户的提问习惯、意图复杂度有明显差距，上线前必须用真实用户 Query 做最终验证。
>
>**不要固定 k 值做评估**：不同的 k 值会直接影响指标结果，建议同时评估 k=1/3/5/10 的指标，找到业务场景下「准确度 + 上下文长度」的最优平衡点。
>
>**不要做一次性评估**：知识库更新、用户提问场景变化、模型迭代都会导致检索效果波动，必须搭建周期性的离线评估和线上监控体系。
...
- 上下文相关性
- 答案相关性：答案是否与当前的问题无关
- 忠实度：生成的答案完全基于上下文，相对产生幻觉的情况比例，哪怕是对的，但是没有依据我们的上下文，我们也认为忠实度也是不够的
- 上下文精度（有效数量/总数量）：找的准不准？ 找了五个总信息，只有一个和答案相关（1/5）
- 上下文召回率（关键信息数量/参考答案中关键信息数量）：要求找到3个有效信息， 但是只找到了一个（1/3）

> 示例：“三体的作者是谁？哪一年出版？”  参考答案“刘慈欣，2006年”
>
> 若上下文仅包含“刘慈欣”，未提“2006年”，则召回率 = 50百分制
