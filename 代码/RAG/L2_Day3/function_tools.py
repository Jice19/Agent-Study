...
class MyVectorDBConnector:
    ...
    def add_documents(self, documents, collection_name='demo'):
        ...
        collection.add(
            embeddings=self.get_embeddings_batch(documents),  # 每个文档的向量
            documents=documents,  # 文档的原文
            ids=[str(uuid.uuid4()) for _ in documents]  # 每个文档的id,uuid确保唯一
        )
    ...
    def search(self, query, collection_name='demo', n_results=5):
        ...
        results = collection.query(
            query_embeddings=self.get_embeddings_batch([query]),  # query_embeddings是查询文本的向量表示
